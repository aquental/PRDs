...
export const load: PageServerLoad = async ({ url }) => {
  ...
  const selected = therapistId
    ? allTherapists.find((t) => t.id === therapistId)
    : null;
  ...
  if (selected) {
    const [clinicsRes, patientsRes, expensesRes] = await Promise.all([
      admin
        .from("clinics")
        .select(
          "id,name,cnpj,address,timezone,address_street,address_number,address_complement,address_zip,address_city,address_state,working_hours_start,working_hours_end,created_at",
        )
      ...
      admin
        .from("expenses")
        .select(
          "id,description,amount,frequency,month,due_day,due_date,is_active,notes,color,created_at",
        )
