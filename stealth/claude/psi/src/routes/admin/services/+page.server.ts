...
import {
  ...
  setServiceSwitch,
  type ServiceId,
} from "$lib/server/service-switches";
...
function withTimeout<T>(p: Promise<T> | PromiseLike<T>): Promise<T> {
  return Promise.race([
    ...
    new Promise<never>((_, reject) =>
      setTimeout(
        () => reject(new Error(`Sem resposta (>${TIMEOUT}ms)`)),
        TIMEOUT,
      ),
    ),
  ]);
...
async function checkCep(
  ...
  if (!enabled) {
    return makeCheck(
      ...
      "error",
      0,
      "Kill-switch ativo — desabilitado pelo admin",
      [],
      enabled,
      true,
    );
  ...
  try {
    ...
    return makeCheck(
      ...
      status,
      ms,
      ms > 2000 ? "Latência elevada" : "Operacional",
      [],
      enabled,
      true,
    );
...
async function checkTts(
  ...
  if (!enabled) {
    const logs = await recentLogs(admin, [
      "tts_synthesis",
      "stt_transcription",
    ]).catch(() => []);
    return makeCheck(
  ...
  if (!res.ok) {
    const st: ServiceStatus =
      res.status === 401 || res.status === 403 ? "error" : "warning";
    return makeCheck(
      "tts",
      "API de TTS / STT",
      st,
...
async function checkRedis(enabled: boolean): Promise<ServiceCheck> {
  ...
  try {
    ...
    return makeCheck(
      ...
      status,
      ms,
      ms > 500 ? "Latência elevada" : "Operacional",
      [],
      enabled,
      true,
    );
...
async function checkSupabase(admin: AdminClient): Promise<ServiceCheck> {
  ...
  try {
    ...
    return makeCheck(
      ...
      status,
      ms,
      ms > 1000 ? "Latência elevada" : "Operacional",
      [],
      true,
      false,
    );
