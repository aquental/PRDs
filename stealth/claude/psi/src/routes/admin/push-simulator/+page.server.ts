...
import type { DuePayment, MonthClosure, OperationalSession } from "$lib/core/types";
...
const MOCK_PAYMENTS: DuePayment[] = [
  { id: "e1", description: "Internet fibra", amount: 120, dueDate: new Date().toLocaleDateString("sv"), paid: false },
  { id: "e2", description: "Aluguel sala", amount: 800, dueDate: new Date().toLocaleDateString("sv"), paid: false },
  { id: "e3", description: "Plano de saúde", amount: 450, dueDate: "2026-04-01", paid: false },
];
...
const MOCK_CLOSURE: MonthClosure = {
  ...
  status: "closed",
  log: [
    { action: "closed", at: "2026-05-01T10:00:00Z", by: "Dra. Beatriz Andrade", role: "profissional" },
  ],
  ...
  const results = [
    {
      ...
      description: "Janela: 18h–20h. Só dispara se houver sessões scheduled no dia.",
    ...
    },
    {
      ...
      description: "Janela: 7h–9h. Só dispara se houver contas vencidas ou vencendo hoje.",
      payload: generateDuePaymentsPush(
        simulatedAt(8),
        MOCK_PAYMENTS,
        TZ,
      ),
    },
    {
      ...
      description: "Janela: 8h–10h do dia 1. Só dispara se for o primeiro dia do mês.",
      payload: generateMonthCloseReminderPush(
        firstOfMonthAt(9),
        TZ,
      ),
    },
    {
      ...
      description: "Dispara imediatamente após reabertura. Notifica a outra parte.",
...
function simulatedAt(localHour: number): Date {
  ...
  return new Date(`${dateStr}T${String(localHour).padStart(2, "0")}:00:00-03:00`);
}

function firstOfMonthAt(localHour: number): Date {
  const now = new Date();
  const [yr, mo] = now.toLocaleDateString("sv", { timeZone: TZ }).split("-").map(Number);
  // Use the first day of next month so the reminder refers to the current month
  const nextMo = mo === 12 ? 1 : mo + 1;
  const nextYr = mo === 12 ? yr + 1 : yr;
  return new Date(`${nextYr}-${String(nextMo).padStart(2, "0")}-01T${String(localHour).padStart(2, "0")}:00:00-03:00`);
}
