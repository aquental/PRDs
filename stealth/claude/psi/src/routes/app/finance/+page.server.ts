...
export const load: PageServerLoad = async ({ locals, parent, url }) => {
  ...
  const [
    ...
    locals.supabase
      .from("patients")
      .select("id, name, active, session_fee")
    ...
    locals.supabase
      .from("expenses")
      .select(
        "id, clinic_id, description, amount, frequency, due_day, due_date, is_active, notes, created_at, updated_at",
      )
  ...
  const unpaidSessions = S.filter((s) => s.status === "completed" && !s.paid)
    .map((s) => ({
      id: s.id,
      fee: s.fee ?? 0,
