...
describe('projectMonthlyRevenue — EC-10: negative fee', () => {
	it('treats a negative session_fee as 0, not reducing the total', () => {
		const patients = [
			makePatient({ id: 'p1', session_fee: 200, sessions_per_month: 4, active: true }),
			makePatient({ id: 'p2', session_fee: -100, sessions_per_month: 4, active: true })
		];
	...
	it('returns 0 when all active patients have negative fees', () => {
		const patients = [makePatient({ session_fee: -500, sessions_per_month: 4, active: true })];
...
describe('patientRevenueRanking — EC-10: negative fee', () => {
	it('treats negative session_fee as 0 in monthly projection', () => {
		const patients = [
			makePatient({ id: 'p1', name: 'Alice', session_fee: 100, sessions_per_month: 4, active: true }),
			makePatient({ id: 'p2', name: 'Bob', session_fee: -999, sessions_per_month: 4, active: true })
		];
...
describe('projectMonthlyRevenue', () => {
	...
	it('sums fee × sessions for active patients', () => {
		const patients = [
			makePatient({ id: 'p1', session_fee: 200, sessions_per_month: 4, active: true }),
			makePatient({ id: 'p2', session_fee: 150, sessions_per_month: 2, active: true })
		];
	...
	it('excludes inactive patients', () => {
		const patients = [
			makePatient({ id: 'p1', session_fee: 200, sessions_per_month: 4, active: true }),
			makePatient({ id: 'p2', session_fee: 999, sessions_per_month: 4, active: false })
		];
		expect(projectMonthlyRevenue(patients)).toBe(800);
	});

	it('treats null session_fee as 0', () => {
		expect(projectMonthlyRevenue([makePatient({ session_fee: null, sessions_per_month: 4 })])).toBe(
			0
		);
	});
});
...
describe('actualRevenue — EC-01/EC-03: inverted range', () => {
	it('throws RangeError when from > to', () => {
		expect(() => actualRevenue([], '2024-06-01', '2024-01-31')).toThrow(RangeError);
	});

	it('error message names both dates', () => {
		expect(() => actualRevenue([], '2024-06-01', '2024-01-31')).toThrow(/2024-06-01.*2024-01-31/);
	});

	it('does NOT throw when from === to', () => {
		const entries = [makeEntry({ type: 'revenue', amount: 50, occurred_at: '2024-03-15' })];
		expect(() => actualRevenue(entries, '2024-03-15', '2024-03-15')).not.toThrow();
	});
});
...
describe('actualRevenue', () => {
	it('sums only revenue entries within range', () => {
		const entries = [
			makeEntry({ id: 'e1', type: 'revenue', amount: 100, occurred_at: '2024-01-10' }),
			makeEntry({ id: 'e2', type: 'revenue', amount: 200, occurred_at: '2024-01-31' }),
			makeEntry({ id: 'e3', type: 'expense', amount: 50, occurred_at: '2024-01-15' })
		];
	...
	it('includes boundary dates', () => {
		const entries = [
			makeEntry({ id: 'e1', type: 'revenue', amount: 100, occurred_at: '2024-01-01' }),
			makeEntry({ id: 'e2', type: 'revenue', amount: 200, occurred_at: '2024-01-31' })
		];
		expect(actualRevenue(entries, '2024-01-01', '2024-01-31')).toBe(300);
	});
	...
	it('excludes entries outside range', () => {
		const entries = [
			makeEntry({ id: 'e1', type: 'revenue', amount: 100, occurred_at: '2023-12-31' }),
			makeEntry({ id: 'e2', type: 'revenue', amount: 200, occurred_at: '2024-02-01' })
		];
...
describe('totalExpenses — EC-01/EC-03: inverted range', () => {
	it('throws RangeError when from > to', () => {
		expect(() => totalExpenses([], '2024-06-01', '2024-01-31')).toThrow(RangeError);
	});
});
...
describe('totalExpenses', () => {
	it('sums only expense entries within range', () => {
		const entries = [
			makeEntry({ id: 'e1', type: 'expense', amount: 50, occurred_at: '2024-01-15' }),
			makeEntry({ id: 'e2', type: 'expense', amount: 80, occurred_at: '2024-01-20' }),
			makeEntry({ id: 'e3', type: 'revenue', amount: 999, occurred_at: '2024-01-15' })
		];
...
describe('expensesForPeriod — EC-01: inverted range', () => {
	it('throws RangeError when from > to', () => {
		...
		expect(() => expensesForPeriod(expenses, '2024-06-01', '2024-01-31')).toThrow(RangeError);
	});
	...
	it('throws when from and to are in the same month but from day > to day (same month-count but still ≤)', () => {
		...
		const expenses = [makeExpense({ amount: 1000, frequency: 'monthly' })];
		expect(() => expensesForPeriod(expenses, '2024-01-15', '2024-01-01')).not.toThrow();
	});
});
...
describe('expensesForPeriod', () => {
	...
	it('one_time inside period counts once', () => {
		const expenses = [
			makeExpense({ amount: 500, frequency: 'one_time', due_date: '2024-01-15' })
		];
	...
	it('one_time outside period is 0', () => {
		const expenses = [
			makeExpense({ amount: 500, frequency: 'one_time', due_date: '2024-02-15' })
		];
	...
	it('one_time with no due_date is 0', () => {
		const expenses = [
			makeExpense({ amount: 500, frequency: 'one_time', due_date: undefined, due_day: null })
		];
		expect(expensesForPeriod(expenses, '2024-01-01', '2024-01-31')).toBe(0);
	});

	it('inactive expense is skipped', () => {
		const expenses = [makeExpense({ amount: 1000, frequency: 'monthly', is_active: false })];
		expect(expensesForPeriod(expenses, '2024-01-01', '2024-01-31')).toBe(0);
	});
...
describe('periodProfit', () => {
	it('revenue − finance expenses − recurring expenses', () => {
		const entries = [
			makeEntry({ id: 'e1', type: 'revenue', amount: 1000, occurred_at: '2024-01-15' }),
			makeEntry({ id: 'e2', type: 'expense', amount: 200, occurred_at: '2024-01-15' })
		];
		const expenses = [makeExpense({ amount: 300, frequency: 'monthly' })];
		// 1000 - 200 - 300 = 500
		expect(periodProfit(entries, expenses, '2024-01-01', '2024-01-31')).toBe(500);
	});

	it('returns negative when expenses exceed revenue', () => {
		const entries = [makeEntry({ type: 'revenue', amount: 100, occurred_at: '2024-01-15' })];
		const expenses = [makeExpense({ amount: 500, frequency: 'monthly' })];
		expect(periodProfit(entries, expenses, '2024-01-01', '2024-01-31')).toBe(-400);
	});
});
...
describe('outstandingRevenue', () => {
	...
	it('treats null fee as 0', () => {
		expect(
			outstandingRevenue([makeSession({ status: 'completed', paid: false, fee: null })])
		).toBe(0);
	});
...
describe('patientRevenueRanking', () => {
	it('ranks active patients by projected monthly revenue descending', () => {
		const patients = [
			makePatient({ id: 'p1', name: 'Alice', session_fee: 100, sessions_per_month: 2, active: true }), // 200
			makePatient({ id: 'p2', name: 'Bob', session_fee: 200, sessions_per_month: 4, active: true }), //  800
			makePatient({ id: 'p3', name: 'Carol', session_fee: 150, sessions_per_month: 3, active: true }) //  450
		];
		const ranking = patientRevenueRanking(patients);
	...
	it('excludes inactive patients', () => {
		const patients = [
			makePatient({ id: 'p1', active: true, session_fee: 100, sessions_per_month: 4 }),
			makePatient({ id: 'p2', active: false, session_fee: 999, sessions_per_month: 4 })
		];
		const ranking = patientRevenueRanking(patients);
	...
	it('includes name in each entry', () => {
		const patients = [makePatient({ id: 'p1', name: 'Alice', session_fee: 100, sessions_per_month: 2 })];
